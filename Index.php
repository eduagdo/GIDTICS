...
require_login ();
$baseurl = new moodle_url ( '/local/GIDTICS/index.php' ); // clase pagina
$context = context_system::instance ();
$PAGE->set_context ( $context );
$PAGE->set_url ( $baseurl );
$PAGE->set_pagelayout ( 'standard' );
//$PAGE->set_title ( get_string ( 'title', 'local_GIDTICS' ) );
$PAGE->set_title ( 'Teachers Connection' );
$PAGE->set_heading ( get_string ( 'title', 'local_GIDTICS' ) );
$PAGE->navbar->add ( get_string ( 'title', 'local_GIDTICS' ) );
// $PAGE->navbar->add('index','reservar.php');
echo $OUTPUT->header (); // shows header
                         // <link rel="stylesheet" href="css/style.css">
echo $OUTPUT->heading ( get_string ( 'title', 'local_GIDTICS' ) );
...
$sql = "SELECT * from {local_GIDTICS} order by Nombre asc";
$resultado = $DB->get_records_sql ( $sql );
$data = '';
foreach ( $resultado as $resultados ) {
	$editurl = new moodle_url ( '/local/GIDTICS/edit.php', array (
			'id' => $resultados->id 
	) );
	$deleteurl = new moodle_url ( '/local/GIDTICS/delete.php', array (
			'id' => $resultados->id 
	) );
	$data [] = array (
			$resultados->nombre,
...
$inventariourl = new moodle_url ( '/local/GIDTICS/guardar.php');
